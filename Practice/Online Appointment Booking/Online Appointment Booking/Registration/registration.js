...
function saveUser(formData) {
  ...
  const duserJSON = JSON.stringify(formData);

  // Save the user data in the local storage using the username as the key
  localStorage.setItem(formData.username, duserJSON);
  }
  else{
     // Convert the user data to a JSON string
  const puserJSON = JSON.stringify(formData);

  // Save the user data in the local storage using the username as the key
  localStorage.setItem(formData.username, puserJSON);
  }
}
